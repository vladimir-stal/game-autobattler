...
export const dealDamage = (target: IBattleUnit, damageValue: number, damageType: EHeroAttackType, battleRecord: TBattleRecord) => {
    ...
    if (divineShield && divineShield.totalValue) {
...
export const applyDebuff = (target: IBattleUnit, debuff: IDebuff, debuffAction: IBattleAction, battleCtrl: BattleController, caster?: IBattleUnit) => {
    ...
    if (existingDebuff) {
        //console.log("existing debuff found", target.id, type, attribute, timeType);
        if (existingDebuff.totalValue === undefined) {
            console.log("ERROR applyDebuff existingDebuff.totalValue undefined");
            return;
        }
        ...
        switch (type) {
            ...
            case EDebuffType.ATTRIBUTE_DECREASE:
                if (!attribute) {
                    console.log("ERROR applyDebuff no attribute");
                    return;
                }
                // caster & battleCtrl should be defined
                //const initValue = debuff.valueFrom ? target[debuff.valueFrom] : target[debuff.attribute];
                const newValue = calculateDebuffValue(caster || target, target[attribute], debuff);
                const oldValue = existingDebuff.totalValue;
                if (debuff.timeType === EBuffTimeType.DURATION && existingDebuff.duration && duration && oldValue) {
...
export const applyBuff = (target: IBattleUnit, buff: IBuff, buffAction: IBattleAction, battleCtrl: BattleController, caster?: IBattleUnit) => {
    ...
    const { attribute, duration, timeType, type, valueFrom } = buff;
    ...
    if (existingBuff) {
        ...
        if (type === EBuffType.ATTRIBUTE_INCREASE) {
            if (!attribute) {
                console.log("ERROR applyBuff no attribute");
                return;
            }
            // caster & battleCtrl should be defined
            const initValue = valueFrom ? target[valueFrom] : attribute ? target[attribute] : 0;
            ...
            if (oldValue !== undefined && timeType === EBuffTimeType.DURATION && existingBuff.duration && duration) {
                ...
                if (existingBuff.totalValue === undefined) {
                    console.log("ERROR applyBuff existingDebuff.totalValue undefined");
                    return;
                }
            ...
            const newValue = calculateBuffValue(caster || target, initValue, buff);
            const oldValue = existingBuff.totalValue;
            if (timeType === EBuffTimeType.DURATION && oldValue !== undefined && existingBuff.duration && duration) {
                existingBuff.totalValue = Math.max(newValue, oldValue);
                existingBuff.duration += duration;
            } else {
